 ...
 *  @style    | [string]  | ('')                   | 加载打印时的额外样式，如果为空，则仅加载原页面样式，如果为css文件，则加载到打印页面的head部分，如果为类名，则在打印页面body标签增加该类
...
(function ($) {
    ...
    $.fn.printArea = function (options) {
        ...
        settings.hasStyleFile = settings.style.toLowerCase().indexOf(".css", settings.style.length - 4) != -1;
        settings.hasStyleClass = (!settings.hasStyleFile) && settings.style != undefined && settings.style != "";
    ...
    function getHead() {
        ...
        if (settings.hasStyleFile) {
            head += '<link type="text/css" rel="stylesheet" href="' +settings.style + '" >';
        }
        return head += "</head>";
    }

    function getBody(printElement) {
        return '<body class="'+(settings.hasStyleClass?settings.style:'')+'"><div class="' + $(printElement).attr("class") + '">' + $(printElement).html() + '</div></body>';
    }
